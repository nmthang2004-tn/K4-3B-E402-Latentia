...
**Lớp:** 3B · **Phòng:** E402 · **Cụm:** C1 · **Track:** B1 

## Thông tin nhóm

| Họ và Tên | Mã HV | Vai trò chính | Phần việc đảm nhiệm chi tiết |
|---|---|---|---|
| **Nguyễn Minh Thắng** | 2A202602706 | **Nhóm trưởng**<br>(Product Lead) | • Chủ trì xây dựng tài liệu đặc tả `spec.md` (§1–§9) & Lean Canvas `canvas.md`<br>• Khai thác dữ liệu thực nghiệm (Evidence Mining): phân tích 142/306 chatlog K4 để chốt bài toán cốt lõi<br>• Điều phối tiến độ nhóm CP1–CP5, biên soạn bộ Slide thuyết trình (`demo-slides.pdf`) |
| **Nguyễn Thị Vàng** | 2A202602897 | **Thành viên**<br>(Prompt/UX & Eval Lead) | • Thiết kế Core System Prompt (`codebase/prompts/system.md`) và 5 Prompt Templates đường đi<br>• Xây dựng Golden Set 24 test cases (`eval/golden_set.json`), thiết lập Quality Bar và chạy kiểm thử tự động<br>• Triển khai khảo sát nhu cầu học viên n = 14 (`eval/survey_analysis.md`) và điều phối thử nghiệm 3 willing users (`validation/feedback_log.md`) |
| **Nguyễn Minh Tuấn** | 2A202602420 | **Thành viên**<br>(Tech & Demo Lead) | • Xây dựng Web Prototype HTML/JS tương tác (`codebase/prototype/index.html`)<br>• Lập trình Bot Discord (`codebase/bot.py`), tích hợp Google Gemini API (`codebase/ai_client.py`) kèm router 5 đường đi<br>• Xây dựng hệ thống logging (`codebase/logger.py`), runner kiểm thử `eval/run_eval.py` & công cụ trích xuất log validation<br>• Vận hành live Discord Bot và phụ trách kịch bản Demo trực tiếp vòng thi CP6 |
